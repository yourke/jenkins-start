...
# 启动参数
start_command=$*
# war变量值，直接使用字符串，避免和jenkins变量冲突
war_var="warVar";
if [ "${start_command}" = "" ]; then
    echo " > 启动命令为空，使用默认值"
    # 默认启动命令
    start_command="java -jar -Xmx1g -Xms1g ${war_var}";
else
    echo " > 启动命令: ${start_command}"
    # 命令校验，需包含包名变量，以便动态替换
    if !([[ ${start_command} == *${war_var}* ]]); then
        echo " > 启动命令中需包含包名变量\"${war_var}\"，用于动态替换包名。中止运行！"
        exit 1;
    fi
fi
...
# 替换包名变量
start_command="${start_command/${war_var}/${package_path}}"
echo " > 执行启动命令：nohup ${start_command} >/dev/null 2>&1 &"
